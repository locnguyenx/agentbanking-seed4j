...
**Version:** 2.0
**Date:** 2026-04-06
**Status:** Revised — Temporal Saga, Tier 4 Translation Services, Spring Boot 4
...
### Business Purpose
Enable third-party retail agents (merchants) to offer banking services — withdrawals, deposits, transfers, bill payments, e-KYC onboarding — to customers via Android/Flutter POS terminals, in compliance with Bank Negara Malaysia (BNM) regulations.
...
### Deliverables
1. **Backend Platform** — 10+ bounded contexts:
   - **Transaction Orchestrator** — Temporal-based Saga coordinator for complex financial flows
   - **Rules & Parameter Service** — Fee engine, limits, velocity checks
   ...
   - **Switch Adapter Service** — Routing logic for external financial rails (PayNet, DuitNow)
   - **Biller Service** — Utility payments, biller webhooks, telco top-ups
   - **ISO Translation Engine** (Tier 4) — JSON to ISO 8583/20022 translation, persistent TCP sockets
   - **CBS Connector** (Tier 4) — Core Banking System legacy protocol bridge (SOAP/MQ/Fixed-Length)
   - **HSM Wrapper** (Tier 4) — Hardware Security Module communication, PIN block translation
   - **Biller Gateway** (Tier 4) — Multi-biller API normalization (JomPAY, Fiuu, TNB, Astro)
   - Clear separation of:
     - **External API (Gateway)** — Spring Cloud Gateway exposing REST endpoints for POS terminals. Documented in public OpenAPI 3.0 spec.
     - **Internal APIs** — Inter-service communication via OpenFeign/Kafka. Orchestrator coordinates all transaction flows via Temporal Saga.
...
- **Temporal Workflows** — Saga orchestration for multi-step financial transactions
- **Synchronous** — OpenFeign with Resilience4j circuit breakers (within Temporal Activities)
- **Asynchronous** — Kafka for non-critical flows (SMS, commissions, EFM)
...
3. Comply with Bank Negara Malaysia's Agent Banking guidelines
...
- EOD Net Settlement & Reconciliation (via bash scripts)
...
- Translation Layer Services (ISO Engine, CBS Connector, HSM Wrapper, Biller Gateway)
...
|----|-----------|-------------|-------|
| US-TO01 | As the system, I want the Transaction Orchestrator to use Temporal to coordinate multi-step financial sagas with automatic retry and compensation | 100% STP | MVP |
| US-TO02 | As the system, I want Temporal to persist saga state so that if the Orchestrator crashes mid-transaction, it can resume from the last completed step | 100% STP | MVP |
...
| ID | User Story | STP Category | Phase |
|----|-----------|-------------|-------|
| US-V01 | As the system, I want Temporal to trigger an MTI 0400 reversal if a network timeout occurs after float was blocked, with durable Store & Forward | 100% STP | MVP |
...
### Translation Layer (Tier 4)
| ID | User Story | STP Category | Phase |
|----|-----------|-------------|-------|
| US-ISO01 | As the system, I want the ISO Translation Engine to convert internal JSON to ISO 8583/20022 and manage persistent TCP connections to PayNet | 100% STP | Phase 2 |
| US-CBS01 | As the system, I want the CBS Connector to translate between internal JSON and Core Banking legacy protocols (SOAP/MQ/Fixed-Length) | 100% STP | Phase 2 |
| US-HSM01 | As the system, I want the HSM Wrapper to handle all PIN block translation so that Tier 2 services never see raw encryption keys | 100% STP | MVP |
| US-BG01 | As the system, I want the Biller Gateway to normalize multiple biller APIs (JomPAY, Fiuu, TNB, Astro) into a unified interface | 100% STP | Phase 2 |

### Error Mapping
| ID | User Story | STP Category | Phase |
|----|-----------|-------------|-------|
| US-EM01 | As the system, I want Tier 4 to normalize legacy error codes (ISO 8583, CBS, HSM) into clean business exceptions for Tier 2 | 100% STP | MVP |

### EOD Bash Settlement
| ID | User Story | STP Category | Phase |
|----|-----------|-------------|-------|
| US-EOD01 | As the system, I want EOD settlement executed via bash scripts directly against PostgreSQL for performance and reliability | N/A | Phase 2 |
...
| ID | Requirement | US |
|----|------------|-----|
| FR-3.1 | System shall process ATM card withdrawals with EMV chip + PIN verification, coordinated via Temporal Saga | US-L05 |
...
| FR-3.4 | System shall trigger MTI 0400 Reversal via Temporal compensation if terminal printer fails or network drops after switch approval | US-L05, US-L06, US-V01 |
| FR-3.5 | System shall debit agent float and credit bank settlement account on withdrawal | US-L05 |

### FR-4: Cash Deposit
| ID | Requirement | US |
|----|------------|-----|
| FR-4.1 | System shall process cash deposits with account validation before funds move, coordinated via Temporal Saga | US-L07 |
...
| ID | Requirement | US |
|----|------------|-----|
| FR-7.1 | System shall process JomPAY bill payments (ON-US and OFF-US) via cash and card, coordinated via Temporal Saga | US-B01, US-D02 |
...
| ID | Requirement | US |
|----|------------|-----|
| FR-9.1 | System shall process DuitNow transfers via ISO 20022 (PayNet), coordinated via Temporal Saga | US-D01 |
...
| ID | Requirement | US |
|----|------------|-----|
| FR-12.1 | Gateway shall route all external POS requests to the Transaction Orchestrator | US-G01 |
| FR-12.2 | Gateway shall authenticate all external requests via OAuth2 token-based auth | US-G02 |
...
| ID | Requirement | US |
|----|------------|-----|
| FR-18.1 | System shall trigger MTI 0400 reversal automatically via Temporal compensation if network timeout occurs after float was blocked (Store & Forward) | US-V01 |
...
| FR-18.4 | Financial activities shall use ZERO retries at the Orchestrator level; timeout shall trigger immediate Temporal compensation | US-V01 |
| FR-18.5 | Non-financial requests (echo, inquiry) shall use exponential backoff retry (1s, 2s, 4s) with max 3 retries | N/A |

### FR-19: Transaction Orchestrator (Temporal Saga)
| ID | Requirement | US |
|----|------------|-----|
| FR-19.1 | System shall use Temporal Workflows to coordinate multi-step financial sagas (Rules check → BlockFloat → Switch Auth → Commit/Rollback) | US-TO01 |
| FR-19.2 | Each saga step shall be implemented as a Temporal Activity with explicit timeout and retry policies | US-TO01 |
| FR-19.3 | Temporal shall persist workflow state so that if the Orchestrator crashes, the saga resumes from the last completed step | US-TO02 |
| FR-19.4 | Each saga step shall have a corresponding compensation handler that reverses the step's effects on failure | US-TO01 |
| FR-19.5 | The Orchestrator shall route all external POS transaction requests to the appropriate Temporal workflow | US-G01 |
| FR-19.6 | Financial activities (BlockFloat, Commit, Rollback) shall use ZERO retries — timeout triggers compensation immediately | US-V01 |
| FR-19.7 | Non-financial activities (Rules check, velocity) shall use exponential backoff retry (max 3 attempts) | N/A |

### FR-20: ISO Translation Engine (Tier 4)
| ID | Requirement | US |
|----|------------|-----|
| FR-20.1 | System shall convert internal JSON to ISO 8583 binary bitmaps for card transactions | US-ISO01 |
| FR-20.2 | System shall convert internal JSON to ISO 20022 XML for DuitNow transfers | US-ISO01 |
| FR-20.3 | System shall maintain persistent TCP/IP socket connections to PayNet with Echo/Heartbeat every 30-60 seconds | US-ISO01 |
| FR-20.4 | System shall generate and track System Trace Audit Number (STAN) incrementally (000001-999999) | US-ISO01 |
| FR-20.5 | System shall unmarshal binary/ISO responses back to clean JSON for Tier 2 consumption | US-ISO01 |

### FR-21: CBS Connector (Tier 4)
| ID | Requirement | US |
|----|------------|-----|
| FR-21.1 | System shall translate internal JSON to Core Banking System legacy protocols (SOAP/MQ/Fixed-Length) | US-CBS01 |
| FR-21.2 | System shall manage Request/Reply Queue orchestration for CBS communication | US-CBS01 |
| FR-21.3 | System shall handle long CBS timeouts without blocking Tier 2 threads | US-CBS01 |
| FR-21.4 | System shall convert settlement JSON to the specific "Flat-File" format for CBS batch engine | US-CBS01 |

### FR-22: HSM Wrapper (Tier 4)
| ID | Requirement | US |
|----|------------|-----|
| FR-22.1 | System shall handle all communication with the Hardware Security Module via proprietary TCP socket commands | US-HSM01 |
| FR-22.2 | System shall translate PIN blocks from Zone Personal Key (ZPK) to Local Master Key (LMK) | US-HSM01 |
| FR-22.3 | System shall verify PIN blocks via HSM | US-HSM01 |
| FR-22.4 | System shall ensure no encryption keys exist in Tier 2 application memory — keys reside only in HSM | US-HSM01 |

### FR-23: Biller Gateway (Tier 4)
| ID | Requirement | US |
|----|------------|-----|
| FR-23.1 | System shall provide a unified interface for multiple biller providers (JomPAY, Fiuu, TNB, Astro) | US-BG01 |
| FR-23.2 | System shall route requests to the correct 3rd party URL based on Biller_ID | US-BG01 |
| FR-23.3 | System shall inject 3rd party API keys and OAuth tokens automatically | US-BG01 |
| FR-23.4 | System shall ensure idempotency at the biller level (prevent double-charging on retry) | US-BG01 |

### FR-24: Error Mapping (Tier 4 Normalization)
| ID | Requirement | US |
|----|------------|-----|
| FR-24.1 | Tier 4 shall normalize legacy error codes (ISO 8583, CBS, HSM) into clean business exceptions for Tier 2 | US-EM01 |
| FR-24.2 | Each normalized error shall include: business error code, human-readable message, and action category (NOTIFY_CUSTOMER, TRIGGER_REVERSAL, STOP_ALERT) | US-EM01 |
| FR-24.3 | Unknown legacy error codes shall map to a safe fallback (ERR_DOWNSTREAM_UNAVAILABLE, action: RETRY) | US-EM01 |

### FR-25: EOD Bash Settlement
| ID | Requirement | US |
|----|------------|-----|
| FR-25.1 | System shall execute EOD settlement via bash scripts directly against PostgreSQL instances | US-EOD01 |
| FR-25.2 | Bash scripts shall aggregate UNSETTLED commissions and generate CSV files for CBS upload | US-EOD01 |
| FR-25.3 | Bash scripts shall run at 23:59:59 MYT and complete settlement file generation by 02:00 AM | US-EOD01 |
...
| sagaExecutionId | String(64) | Conditional | Temporal workflow execution ID |
...
| netCreditToFloat | BigDecimal(15,2) | Yes | Net amount credited to agent float |
...
| NFR-1.4 | Temporal Activity timeout for financial operations: 25 seconds max |
| NFR-1.5 | Temporal Activity timeout for non-financial operations: 5 seconds max |
...
| NFR-2.4 | Temporal workflow persistence ensures saga recovery across Orchestrator crashes |

### NFR-3: Security
| ID | Requirement |
|----|------------|
| NFR-3.1 | Zero-trust architecture — every request authenticated, every service authorized |
| NFR-3.2 | PINs must never be logged in plaintext — hardware-level encryption via HSM |
...
| NFR-3.6 | Tier 2 services must never see raw encryption keys — all crypto operations via HSM Wrapper |

### NFR-4: Compliance
| ID | Requirement |
|----|------------|
| NFR-4.1 | Comply with Bank Negara Malaysia Agent Banking guidelines |
...
| ID | Requirement |
|----|------------|
| NFR-5.1 | Each bounded context independently scalable |
| NFR-5.2 | Support horizontal scaling via Kubernetes (future) |
| NFR-5.3 | Temporal Server supports workflow sharding for high-throughput saga execution |
...
| NFR-6.4 | Micrometer metrics exposed on /actuator/metrics for all services |
| NFR-6.5 | Temporal workflow execution history available for audit and debugging |

### NFR-7: Temporal Durability
| ID | Requirement |
|----|------------|
| NFR-7.1 | Temporal workflows shall persist state after each saga step completion |
| NFR-7.2 | Temporal shall automatically resume interrupted workflows from the last completed step |
| NFR-7.3 | Compensation handlers shall execute for all completed steps when a saga fails |
| NFR-7.4 | Temporal event history shall be retained for minimum 30 days for audit purposes |
...
| C-2 | Framework: Spring Boot 4, Spring Cloud only |
...
| C-13 | Saga Orchestration: Temporal for all multi-step financial transactions |
| C-14 | IAM: OAuth2 / Keycloak for authentication and authorization |
| C-15 | Monitoring: Micrometer for all service metrics |
...
| A-5 | **EOD net settlement batch** — at 23:59:59 MYT, bash scripts aggregate daily activity into a single net figure per agent and generate a settlement file for CBS upload. Actual money movement to the agent's real bank account happens via this EOD batch |
| A-6 | Three agent tiers: Micro, Standard, Premier |
| A-7 | Backoffice UI framework is React + TypeScript + Vite |
| A-8 | Infrastructure (Kubernetes, CI/CD) is out of scope for initial specs |
| A-9 | Temporal Server is available as infrastructure (Docker Compose for local dev, Helm for production) |
| A-10 | All transaction flows go through the Transaction Orchestrator — no direct peer-to-peer service calls for financial operations |
...
|-----------|------------------------|-------------|-------|
| US-TO01 | FR-19.1, FR-19.2, FR-19.4, FR-19.6, FR-19.7 | 100% STP | MVP |
| US-TO02 | FR-19.3 | 100% STP | MVP |
...
| US-L05 | FR-2.4, FR-3.1, FR-3.3, FR-3.4, FR-3.5, FR-18.1, FR-19.1 | 100% STP | MVP |
| US-L06 | FR-3.2, FR-3.3, FR-3.4, FR-3.5 | 100% STP | Phase 2 |
| US-L07 | FR-2.5, FR-4.1, FR-4.3, FR-19.1 | 100% STP | MVP |
...
| US-B01 | FR-7.1, FR-19.1 | 100% STP | Phase 2 |
...
| US-V01 | FR-18.1, FR-18.2, FR-18.4, FR-19.4 | 100% STP | MVP |
...
| US-ISO01 | FR-20.1, FR-20.2, FR-20.3, FR-20.4, FR-20.5 | 100% STP | Phase 2 |
| US-CBS01 | FR-21.1, FR-21.2, FR-21.3, FR-21.4 | 100% STP | Phase 2 |
| US-HSM01 | FR-22.1, FR-22.2, FR-22.3, FR-22.4 | 100% STP | MVP |
| US-BG01 | FR-23.1, FR-23.2, FR-23.3, FR-23.4 | 100% STP | Phase 2 |
| US-EM01 | FR-24.1, FR-24.2, FR-24.3 | 100% STP | MVP |
| US-EOD01 | FR-25.1, FR-25.2, FR-25.3 | N/A | Phase 2 |
