...
|--------|-------------|------------|-------------|
| `init` | - | `projectName`, `baseName`, `nodePackageManager`, `endOfLine`, `indentation`, `springConfigurationFormat` | Init project |
| `prettier` | `module:init` | `endOfLine`, `indentation` | Format project with prettier |
...
| Module | Dependencies | Parameters | Description |
|--------|-------------|------------|-------------|
| `maven-java` | `module:init` | `packageName` | Init Maven project with pom.xml |
| `maven-wrapper` | `module:maven-java` | - | Add maven wrapper |
| `gradle-java` | `module:init` | `packageName` | Init Gradle project with kotlin DSL |
...
| Module | Dependencies | Parameters | Description |
|--------|-------------|------------|-------------|
| `spring-boot` | `feature:java-build-tool`, `module:java-base` | `serverPort` | Init Spring Boot project with dependencies, App, and properties |
...
| `gateway` | `module:spring-boot-webflux-empty`, `module:spring-cloud` | `basePackage`, `baseName` | Add Spring Cloud Gateway |
...
```
# ARCHITECTUREAL KNOWLEDGE

## Business Context
@BusinessContext is NOT a standard Spring Boot annotation. It's a custom annotation defined in the Agent Banking project that follows the hexagonal architecture principles.

**What it is:**
- A custom Java annotation used to mark package boundaries in the hexagonal architecture
- Found in src/main/java/com/agentbanking/BusinessContext.java
- Used in package-info.java files to annotate entire packages

**Why it's needed:**
1. Architecture Enforcement: ArchUnit tests use @BusinessContext to verify:
   - Domain layer doesn't depend on application layer
   - Controllers only depend on application layer (not domain)
   - Proper separation between bounded contexts
2. Bounded Context Identification: Clearly marks which packages belong to which business domain (biller, ledger, onboarding, etc.)
3. Hexagonal Architecture Compliance: Required by the project's architectural laws to maintain clean separation between domain, application, and infrastructure layers
4. Shared Kernel Detection: Works with @SharedKernel annotation to identify code shared between contexts

**Example usage:**
// src/main/java/com/agentbanking/biller/package-info.java
@BusinessContext
package com.agentbanking.biller;
This tells ArchUnit and developers that everything in the com.agentbanking.biller package belongs to the biller bounded context and must follow the hexagonal architecture rules for that context.
The annotation enables automated architecture validation to prevent dependency violations that would break the modular monolith structure.

**Key Findings About Seed4J CLI:**
- The apply bounded-context --context <name> command does not exist in standard Seed4J CLI
- Bounded contexts must be created manually using the hexagonal architecture structure
- The jmolecules module provides @BoundedContext annotation support but doesn't create package structure
- Plans referencing apply bounded-context require manual implementation of the directory structure