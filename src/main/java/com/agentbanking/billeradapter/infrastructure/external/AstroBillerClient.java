...
@Component
public class AstroBillerClient {

    private static final Logger log = LoggerFactory.getLogger(AstroBillerClient.class);
    private static final int TIMEOUT_SECONDS = 30;
    private final ExecutorService executor = Executors.newCachedThreadPool();

    public BillDetails getBillDetails(String ref1, String ref2) {
        log.info("Calling ASTRO bill inquiry API for ref: {}", ref1);
        
        try {
            Future<BillDetails> future = executor.submit(() -> simulateGetBillDetails(ref1));
            return future.get(TIMEOUT_SECONDS, TimeUnit.SECONDS);
        } catch (TimeoutException e) {
            log.error("ASTRO bill inquiry timeout");
            throw new BillerTimeoutWrapper.BillerTimeoutException("ERR_EXT_201: ASTRO timeout");
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("ERR_EXT_201: ASTRO interrupted");
        } catch (ExecutionException e) {
            throw new RuntimeException("ERR_EXT_202: ASTRO error", e.getCause());
        }
    }
    ...
    private BillDetails simulateGetBillDetails(String ref1) {
        ...
        return new BillDetails("ASTRO", ref1, "CUSTOMER NAME", new BigDecimal("150.00"), "Astro Subscription");
    }

    public PaymentResult payBill(String ref1, String ref2, BigDecimal amount) {
        log.info("Calling ASTRO payment API for ref: {}, amount: {}", ref1, amount);
        
        try {
            Future<PaymentResult> future = executor.submit(() -> simulatePayBill(ref1, amount));
            return future.get(TIMEOUT_SECONDS, TimeUnit.SECONDS);
        } catch (TimeoutException e) {
            log.error("ASTRO payment timeout");
    ...
    private PaymentResult simulatePayBill(String ref1, BigDecimal amount) {
        ...
        return new PaymentResult(true,
            ...
            "ASTRO-" + UUID.randomUUID().toString().substring(0, 6).toUpperCase(), null);
    }

    public ReversalResult reverseBill(String transactionId, String reason) {
        log.info("Calling ASTRO reversal API for transaction: {}", transactionId);
