...
public class JompayBillerClient {
    ...
    public BillDetails getBillDetails(String ref1, String ref2) {
        ...
        try {
            ...
            log.error("JomPAY bill inquiry timeout");
            throw new BillerTimeoutWrapper.BillerTimeoutException("ERR_EXT_201: JomPAY timeout");
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException("ERR_EXT_201: JomPAY interrupted");
        } catch (ExecutionException e) {
            throw new RuntimeException("ERR_EXT_202: JomPAY error", e.getCause());
        }
    }
    ...
    public PaymentResult payBill(String ref1, String ref2, BigDecimal amount) {
        ...
        try {
            ...
            log.error("JomPAY payment timeout");
    ...
    public ReversalResult reverseBill(String transactionId, String reason) {
        log.info("Calling JomPAY reversal API for transaction: {}", transactionId);
        return new ReversalResult(false, null, "Reversal not supported for JomPAY");
    }
}