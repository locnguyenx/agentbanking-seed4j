...
@Component
public class TmBillerClient {

  private static final Logger log = LoggerFactory.getLogger(TmBillerClient.class);

  public BillDetails getBillDetails(String ref1, String ref2) {
    log.info("Calling TM bill inquiry API for ref: {}", ref1);
    return new BillDetails("TM", ref1, "CUSTOMER NAME", new BigDecimal("200.00"), "TM Bills");
  }

  public PaymentResult payBill(String ref1, String ref2, BigDecimal amount) {
    log.info("Calling TM payment API for ref: {}, amount: {}", ref1, amount);
    return new PaymentResult(true,
        "TXN-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(),
        "TM-" + UUID.randomUUID().toString().substring(0, 6).toUpperCase(), null);
  }

  public ReversalResult reverseBill(String transactionId, String reason) {
    log.info("Calling TM reversal API for transaction: {}", transactionId);
